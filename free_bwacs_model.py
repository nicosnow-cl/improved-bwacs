        ...
        while unvisited_nodes.size:
            ...
            while valid_nodes.size:
                ...
                if q <= self.q0:
                    ...
                    s = self.random.choices(valid_nodes, cum_weights = cum_sum, k = 1)[0]
                ...
                _demands_by_node = self.demands_array[unvisited_nodes] + vehicle_capacity
                valid_unvisited_nodes = unvisited_nodes[_demands_by_node <= self.vehicle_capacity]                
                valid_unvisited_nodes_sorted = self.distances_matrix[r][valid_unvisited_nodes].argsort()[:max_node]
...
class FreeBWACS:
    ...
    def printInstance(self):
        ...
        for i in self.clients:
            self.plt.plot(self.coords_matrix[i][0] * 0.2, self.coords_matrix[i][1] * 0.2, c = self.color_palette[0], marker = 'o', markersize = 17)
            self.plt.annotate('$q_{%d}=%d$'%(i, self.demands_array[i]), (self.coords_matrix[i][0] * 0.2, (self.coords_matrix[i][1]-3)* 0.2))  
                        
        self.plt.plot(self.loc_x[self.depot] * 0.2, self.loc_y[self.depot] * 0.2, c = 'r', marker = 's', markersize = 17)
        self.plt.annotate('DEPOT', ((self.loc_x[self.depot]-1.5)* 0.2, (self.loc_y[self.depot]-3)* 0.2))
        ...
        for k, cluster in enumerate(self.clusters):
            for i in cluster:
                if i not in centers_list:
                    self.plt.plot(self.coords_matrix[i][0], self.coords_matrix[i][1], c = self.color_palette[k], marker = 'o', markersize = 17)
                    self.plt.annotate('$q_{%d}=%d$'%(i, self.demands_array[i]), (self.coords_matrix[i][0], self.coords_matrix[i][1]-2))   
                
        for k, j in enumerate(centers_list):
            if self.CLUSTER_TYPE == 'kmedoids':
                self.plt.plot(self.coords_matrix[j][0], self.coords_matrix[j][1], c = self.color_palette[k], marker = '*', markersize = 30)
                self.plt.annotate('$medoid_{%d}$'%(k + 1), (self.coords_matrix[j][0], self.coords_matrix[j][1]-2))
            else:
                self.plt.plot(j[0], j[1], c = self.color_palette[k], marker = '*', markersize = 30)
                self.plt.annotate('$centroid_{%d}$'%(k + 1), (j[0], j[1]-2))
        
        self.plt.plot(self.loc_x[self.depot], self.loc_y[self.depot], c = 'r', marker = 's', markersize = 17)
        self.plt.annotate('DEPOT', (self.loc_x[self.depot]-1.5, self.loc_y[self.depot]-2))
        self.plt.show()
    ...
    def printSolution(self, solution, routes_arcs, solution_energies, solution_distances):
        ...
        for k, route in enumerate(solution):            
            for i in route:              
                if i != 0:
                    self.plt.plot(self.coords_matrix[i][0] * 0.2, self.coords_matrix[i][1] * 0.2, c = self.color_palette[k], marker = 'o', markersize = 17)
                    self.plt.annotate('$q_{%d}=%d$'%(i, self.demands_array[i]), (self.coords_matrix[i][0] * 0.2, (self.coords_matrix[i][1]-3)* 0.2))
            ...
            for x, y in routes_arcs[k]:
                self.plt.plot([self.loc_x[x] * 0.2, self.loc_x[y] * 0.2], [self.loc_y[x] * 0.2, self.loc_y[y] * 0.2], c = self.color_palette[k], alpha = 0.5, linewidth = 4)
    ...
    def printDistancesMatrixNormalized(self):
        from IPython.display import display, HTML        
        pd_distances_matrix = self.pd.DataFrame(self.distances_matrix_normalized)         
        display(HTML(pd_distances_matrix.to_html()))
    ...
    def printSavingEnergyMatrix(self):
        from IPython.display import display, HTML
        pd_saving_energies_matrix = self.pd.DataFrame(self.saving_energies_matrix)
    ...
    def printPheromonesMatrix(self):
        from IPython.display import display, HTML
        pd_pheromones_matrix = self.pd.DataFrame(self.np.power(self.pheromones_matrix, self.alpha))
    ...
    def setInitialParameters(self):
        ...
        if self.MAX_ITERATIONS == None: self.MAX_ITERATIONS = self.math.ceil((self.math.sqrt(len(self.nodes)))*(1/len(self.nodes))*100)        
        self.nodes, self.demands_array = [self.depot] + self.clients, self.np.array(list(self.demands.values()))
        self.MAX_ANTS = self.math.ceil(len(self.clients) / self.TOTAL_ANT_DIVIDER)
        self.coords_matrix = self.np.array([(self.loc_x[i], self.loc_y[i]) for i in self.nodes])
        self.color_palette = cm.jet(self.np.linspace(0, 1, self.K_NUMBER + 1)) # Define a colors palette for the graphs functions.
        self.TARE = self.VEHICLE_CAPACITY * self.TARE_PERCENTAGE # Define the weight of an empty vehicle.
    
    # Function to create a 'ndarray' that represents the matrix of DISTANCES between nodes.
    def createDistancesMatrix(self):
        self.distances_matrix = self.np.zeros((len(self.nodes), (len(self.nodes))))        
        ord_ = 1 if self.METRIC == 'manhattan' else 2
        
        for i in self.nodes:          
            for j in self.nodes:
                if i != j:           
                    self.distances_matrix[i][j] = self.np.linalg.norm(self.coords_matrix[i] - self.coords_matrix[j], ord = ord_)
                # else:
    ...
    def createNormalizedDistancesMatrix(self):
        ...
        self.distances_matrix_normalized = scaler.transform(self.distances_matrix_normalized)
    
    # Function to create a 'ndarray' that represents the matrix of ENERGIES between nodes.
    def createEnergiesMatrix(self):
        self.energies_matrix = self.np.zeros((len(self.nodes), len(self.nodes))) 
        
        for i in self.nodes:
            if i == self.depot: self.energies_matrix[i] = self.np.multiply(self.distances_matrix[i], self.TARE)                
            else: self.energies_matrix[i] = self.np.multiply(self.distances_matrix[i], (self.demands_array[i] + self.TARE))
    ...
    def createNormalizedEnergiesMatrix(self):
        ...
        self.energies_matrix_normalized[self.energies_matrix_normalized == self.np.inf] = 0
        scaler.fit(self.energies_matrix_normalized)
        self.energies_matrix_normalized = scaler.transform(self.energies_matrix_normalized)
    ...
    def createSavingEnergiesMatrix(self):
        self.saving_energies_matrix = self.np.zeros((len(self.nodes), len(self.nodes)))        
        for i in self.nodes[self.depot + 1:]:
            for j in self.nodes[self.depot + 1:]:
                if i != j:                    
        ...
        self.saving_energies_matrix = scaler.transform(self.saving_energies_matrix)
    ...
    def createCapacityUtilizationMatrix(self):
        ...
        for i in self.nodes:
            for j in self.nodes:
                self.cu_matrix[i][j] = (self.demands_array[i] + self.demands_array[j]) / self.VEHICLE_CAPACITY
    
    def normalizePheromonesMatrix(self):
        # Here we normalice the values between 0 and 1.
        from sklearn.preprocessing import MinMaxScaler
        scaler = MinMaxScaler()
        pheromones_matrix_powered = self.np.power(self.pheromones_matrix, self.alpha)
    ...
    def normalizeCombinationMatrix(self):
        ...
        from sklearn.preprocessing import MinMaxScaler
        scaler = MinMaxScaler()
        combination_matrix = self.np.multiply(self._pheromones_matrix, self.combination_matrix)
    ...
    def combineHeuristicMatrices(self):
        self.combination_matrix = self.np.zeros((len(self.nodes), len(self.nodes)))
        
        if (self.HEURISTIC_TYPE == 0):
            _distance_matrix = self.np.divide(1, self.distances_matrix)
            self.combination_matrix = self.np.power(_distance_matrix, self.beta)
            ...
            if self.USE_NORMALIZED_MATRIX:
                _distances_matrix_normalized = self.np.power(self.distances_matrix_normalized, self.beta)
                _energies_matrix_normalized = self.np.power(self.energies_matrix_normalized, self.gamma)   
                self.combination_matrix = self.np.multiply(_distances_matrix_normalized, _energies_matrix_normalized) 
            else:
                ...
                self.combination_matrix = self.np.multiply(_distance_matrix, _energy_matrix) 
        
        elif (self.HEURISTIC_TYPE == 3):
            self.combination_matrix = self.np.power(self.saving_matrix, self.delta)
        
        elif (self.HEURISTIC_TYPE == 4):
            self.combination_matrix = self.np.power(self.saving_energies_matrix, self.eta)
        
        elif (self.HEURISTIC_TYPE == 5):
            _distance_matrix = self.np.power(self.np.divide(1, self.distances_matrix), self.beta)
            _saving_matrix = self.np.power(self.saving_matrix, self.delta)
            self.combination_matrix = self.np.multiply(_distance_matrix, _saving_matrix) 
        
        elif (self.HEURISTIC_TYPE == 6):
            _energy_matrix = self.np.power(self.np.divide(1, self.energies_matrix), self.gamma)
            _saving_energies_matrix = self.np.power(self.saving_energies_matrix, self.eta)
            self.combination_matrix = self.np.multiply(_energy_matrix, _saving_energies_matrix)
            ...
            _saving_matrix = self.np.power(self.saving_matrix, self.delta)
            self.combination_matrix = self.np.multiply(_distance_matrix, _energy_matrix) 
            self.combination_matrix = self.np.multiply(self.combination_matrix, _saving_matrix)
            ...
            self.combination_matrix = self.np.multiply(_distance_matrix, _energy_matrix) 
            self.combination_matrix = self.np.multiply(self.combination_matrix, _saving_matrix)
            self.combination_matrix = self.np.multiply(self.combination_matrix, _cu_matrix)        
    ...
    def getClosestNodesFromDepot(self):
        for cluster in self.clusters:
            ...
            self.closest_nodes_in_clusters.append(cluster[closests_nodes_to_depot[0]])
        ...
        self.closest_nodes_in_clusters = [idx - 1 for idx in self.closest_nodes_in_clusters]
        
    # This function create the new candidate_list.
    def createCandidateList(self, best_nodes = []):
        candidate_list = [] + self.closest_nodes_in_clusters # First select all the closest node to depot for every cluster.
        candidate_list =  candidate_list + best_nodes # best_nodes is the list with the first nodes in the K best solutions in an iteration.
        possible_random_nodes = [node for node in self.clients if node not in candidate_list]
        random_nodes = self.random.sample(possible_random_nodes, self.MAX_ANTS - len(candidate_list)) # Create a list with random nodes.
    ...
    def calculateTminTmax(self):
        ...
        self.t_min = self.H / (energy.sum() * 2) # Define the lower bound for the pheromones value.
        self.t_max = (self.H / energy.sum()) * self.K_NUMBER # Define the upper bound for the pheromones value. 
        self.t_0 = (self.t_min + self.t_max) / 2 # This is the value for the initial pheromones, a middle point between t_min and t_max.
    ...
    def setInitialPheromones(self):
        self.pheromones_matrix = self.np.full((len(self.nodes), len(self.nodes)), self.t_0)  # Start the pheromone's matrix with only t_0 values.     
        for cluster in self.clusters:           
            cluster_arcs = list(self.permutations(cluster, 2))
            print(cluster_arcs)
            for i, j in cluster_arcs:
...
#                 self.pheromones_matrix[i][j] += pheromones_quantity * 0.85
    ...
    def localUpdatePheromones(self, ant_solution, ant_quality):        
        if self.PHEROMONE_UPDATING_STRATEGY == 0:
            ...
            for route_arcs in ant_solution_arcs:
                for i, j in route_arcs:
                    self.pheromones_matrix[i][j] += pheromones_quantity / self.MAX_ANTS
        else:
            ant_solution_arcs = self.generateSolutionArcs(ant_solution)
            total_energy_by_route = []
            energy_arcs_by_route = []
    ...
    def mutatePheromonesMatrix(self, cb_solution_arcs, iteration, last_restart):
        ...
        mutation_1 = (iteration - last_restart) / (self.MAX_ITERATIONS - last_restart)
    ...
    def solve(self):
        ...
        result_list[0], result_list[1], result_list[2], result_list[3], result_list[4], result_list[5], result_list[6], result_list[7], result_list[8]    
        ...
        print('        > Vehicles capacity: ' + str(self.VEHICLE_CAPACITY) + ' units')
        ...
        print('        > Number of ants per iteration: ' + str(self.MAX_ANTS) + '\n')
        ...
        if self.PRINT_CLUSTERS: 
            print('    - Clusters draw:')
            self.printClusters(centers_list) # Want to print the graph of clusters?
            print(self.clusters)
        print('    - Generated Clusters')
        for k in range(self.K_NUMBER):
            total_demand_on_k_cluster = sum([self.demands_array[i] for i in self.clusters[k]])
            ...
            if total_demand_on_k_cluster > self.VEHICLE_CAPACITY:
                ...
                return None
        print('         > Total nodes (without depot): ' + str(sum([len(cluster) for cluster in self.clusters])) + '\n')
        ...
        from src.local_search import Free2OPTSearch # Restricted VNS Local Search only to a single ant solution (only intracluster operators)
        from src.local_search import FreeLocalGVNS # General VNS Local Search (intercluster and intracluster operators)
        
        if self.MUTATE_PHEROMONES_MATRIX: self.createClustersArcsMatrix() # Mutation function depends of this new matrix.
        self.getClosestNodesFromDepot() # We use this for the creation of the candidate_list.
        self.calculateTminTmax()
        self.setInitialPheromones()
        candidate_list = self.createCandidateList() # Create the list of candidate nodes, every node represent the deploy of an Ant.
        ...
        last_restart = 0
        stagnation = 0 # This variable controls when a reinitialization of the pheromone matrix should be done. 
        start_local_search = int(self.MAX_ITERATIONS / 4) # In which iteration will the local search process begin?
                
        for iteration in range(self.MAX_ITERATIONS):
            print('\n    • Iteration ' + str(iteration + 1))            
            mean_quality = 0 # We use this variable to show the mean quality for the all iteration solutions.
            ...
            if self.USE_NORMALIZED_MATRIX: 
                ...
                self._pheromones_matrix = self.np.power(self.pheromones_matrix, self.alpha)
                self._combinations_matrix = self.np.multiply(self._pheromones_matrix, self.combination_matrix)
                from sklearn.preprocessing import MinMaxScaler
                scaler = MinMaxScaler()
                scaler.fit(self._combinations_matrix)
                self._combinations_matrix = scaler.transform(self._combinations_matrix)
            else: 
                self._pheromones_matrix = self.np.power(self.pheromones_matrix, self.alpha)
                self._combinations_matrix = self.np.multiply(self._pheromones_matrix, self.combination_matrix)
            ...
            for node in candidate_list:
                ...
                best_k_quality = [self.np.inf for i in range(int((self.MAX_ANTS * 25) / 100))] 
                best_k_nodes = [0 for i in range(int((self.MAX_ANTS * 25) / 100))]
                ...
                if self.LS_ANT_SOLUTION:
                    ...
                    node = ant_solution[0][1] # We save the first node in the first route of the new solution.
                
                # Want to do an update of the pheromone's matrix with the single ant solution?
                if self.LOCAL_ANT_UPDATE_PHEROMONES: self.localUpdatePheromones(ant_solution, sum(routes_energies))
                ...
                if self.ONLY_K_OPTIMUM:                    
                    if sum(routes_energies) < cb_solution_quality and len(ant_solution) <= self.K_NUMBER:
                        ...
                        cb_solution_arcs = self.generateSolutionArcs(cb_solution)                
                        ...
                        cw_solution_arcs = self.generateSolutionArcs(cw_solution)
                else:
            ...
            candidate_list = self.createCandidateList(best_k_nodes) # We update the candidate_list with the best_k_nodes of this iteration
            print('        > Mean quality of the solutions in this iteration : ' + str(mean_quality / self.MAX_ANTS))
            ...
            else: stagnation += 1 # Increase the stagnation counter in 1 (iteration).             
            ...
            # Else, we update the values of the pheromone's matrix.
                ...
                if self.PENALIZE_WORST_SOLUTION: self.currentWorstUpdatePheromones(cw_solution_arcs) 
                if self.BEST_ITERATION_ANT_UPDATE_PHEROMONES: self.currentBestUpdatePheromones(cb_solution, cb_solution_arcs, cb_solution_quality)
                if self.BEST_GLOBAL_ANT_UPDATE_PHEROMONES: self.globalBestUpdatePheromones(gb_solution, gb_solution_arcs, gb_solution_quality)
                if self.MUTATE_PHEROMONES_MATRIX: self.mutatePheromonesMatrix(cb_solution_arcs, iteration, last_restart)
            
            # Set all the pheromones arcs between the range [t_min, t_max]
            self.pheromones_matrix[self.pheromones_matrix < self.t_min] = self.t_min
            self.pheromones_matrix[self.pheromones_matrix > self.t_max] = self.t_max
                 
        print('\n    • Global best routes:')
        for k, route in enumerate(gb_solution):
            print('        - Route ' + str(k) + ': ' + str(route) + ', with final total demand: ' + str(self.demands_array[route].sum()))
        
        print('        - Final total energy: ' + str(sum(gb_solution_energies)))
        print('        - Final total distance: ' + str(sum(gb_solution_distances)))
        print('        - Total time: %s seconds.' % (time.time() - start_time))        
        
        if self.PRINT_SOLUTION: 
            print('        - Solution draw:')
            self.printSolution(gb_solution, gb_solution_arcs, gb_solution_energies, gb_solution_distances)
            for k, route in enumerate(gb_solution):
                print("Ruta : " + str(k))
