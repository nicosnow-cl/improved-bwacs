...
class GeneralVNS:
    ...
    def improve(
        self, initial_solution: List[int], actual_iteration: int
    ) -> ACOSolution:
        ...
        max_time = (
            ...
            else get_ls_max_time(
                len(self.lst_demands), actual_iteration, self.max_iterations
            )
        )

        neighborhoods_samples = [
            self.single_route_relocate,
