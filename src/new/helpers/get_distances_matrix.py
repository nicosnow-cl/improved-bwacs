...
def get_distances_matrix(nodes, coords_matrix, metric="euclidean"):
    ...
    if metric not in ("euclidean", "manhattan"):
        raise ValueError(
            "Invalid metric: {}. Valid options are 'euclidean' and".format(
                metric
            )
            + " 'manhattan'."
        )
    ...
    if len(nodes) != coords_matrix.shape[0]:
        raise ValueError(
            "Length of nodes ({}) is not compatible with".format(len(nodes))
            + " shape of coords_matrix ({}).".format(coords_matrix.shape)
        )
