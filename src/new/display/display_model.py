...
class DisplayModel:
    ...
    def render_problem(
        ...
        for node in clients:
            ...
            plt.annotate(
                "$q_{%d}=%d$" % (node, demands[node]), (desc_x, desc_y)
            )

        plt.plot(depot_x, depot_y, c="r", marker="s")
        plt.annotate(
            "DEPOT", (depot_x + (depot_x * 0.01), depot_y - (depot_y * 0.03))
        )
        plt.grid()
    ...
    def render_k_means_clusters(
        ...
        for idx, cluster in enumerate(clusters):
            for node in cluster:
                ...
                plt.annotate(
                    f"n{node}", (desc_x, desc_y), alpha=0.3, fontsize=8
                )
        ...
        for idx, centroid in enumerate(centroids):
            ...
            plt.annotate(
                f"$cent_{idx}$", (desc_x, desc_y), alpha=0.6, fontsize=10
            )
        ...
        plt.annotate(
            "DEPOT", (desc_depot_x, desc_depot_y), alpha=0.3, fontsize=8
        )
    ...
    def render_solution(
        ...
        if (
            ...
            or avg_costs is not None
            or median_costs is not None
        ):
            figsize = [14, 6]
        ...
        for idx, route in enumerate(solution["routes"]):
            ...
            for i, j in route_arcs:
                ...
                if i == 0 or j == 0:
                    ...
                    plt.plot((i_x, j_x), (i_y, j_y), c=color_palette[idx])
        plt.grid()
        ...
        if avg_costs is not None:
            ...
            plt.plot(
                iterations, costs, c="g", linewidth=2.0, label="It. Avg Cost"
            )
        ...
        if solutions is not None:
            ...
            plt.plot(
                iterations, costs, c="b", linewidth=2.0, label="It. Best Cost"
            )
            plt.plot(
                it_max_cost + 1, max_cost, c="r", marker="o", label="Max Cost"
            )
            plt.plot(
                it_min_cost + 1, min_cost, c="g", marker="o", label="Min Cost"
            )
    ...
    @staticmethod
    def plot_pheromones_matrices(
        pheromones_matrices: List[np.ndarray],
        name: str,
        max_plots: int = 20,
        output_file: str = None,
    ):
        N = len(pheromones_matrices)
        matrices_idxs = np.linspace(0, N - 1, max_plots, dtype=int)
        cols = 5
        rows = ceil(len(matrices_idxs) / cols)
        size = 4
        ...
        idx_count = 0
        for idx in matrices_idxs:
            matrix = pheromones_matrices[idx]

            ax = axs.flat[idx_count]
            ax.imshow(matrix, interpolation=None, cmap="viridis")
            ax.set_title(f"it={idx}")
        ...
        plt.title(f"Evolution of pheromones matrix ({name})")
    ...
    def plot_most_used_arcs(
        ...
        plt.figure(figsize=figsize)
        plt.title(f"Most used arcs ({name})")

        alpha = 1.0 / len(solutions)

        for solution in solutions:
            for route_arcs in solution["routes_arcs"]:
                for i, j in route_arcs:
                    i_x = matrix_coords[i][0]
                    i_y = matrix_coords[i][1]
                    j_x = matrix_coords[j][0]
                    ...
                    if i == 0 or j == 0:
                        ...
                        plt.plot((i_x, j_x), (i_y, j_y), c="r", alpha=alpha)
        ...
        for node in nodes[1:]:
            ...
            plt.annotate(f"n{node}", (desc_x, desc_y), fontsize=8)
