...
from ..helpers import get_coords_matrix, get_route_arcs


class VRPModel(ProblemModel):
    @staticmethod
    def get_cost_between_two_nodes(
        node1: int, node2: int, matrix_distances: np.ndarray
    ) -> float:
        return matrix_distances[node1][node2]

    @staticmethod
    def fitness_function_legacy(
        route_arcs: List[Tuple[int, int]], matrix_distances: np.ndarray
    ) -> float:
        """
    ...
    @staticmethod
    def get_route_cost(
        route: List[int], matrix_distances: np.ndarray
    ) -> float:
        arcs = get_route_arcs(route)
        route_cost = 0
    ...
    def fitness_function(
        route_arcs: np.ndarray, matrix_distances: np.ndarray
    ) -> float:
    ...
    @staticmethod
    def fitness(
        solution: List[List[int]], matrix_distances: np.ndarray
    ) -> List[float]:
        return [
            __class__.fitness_function(route_arcs, matrix_distances)
    ...
    def validate_instance(nodes, demands, max_capacity) -> dict:
        ...
        if not all(demand >= 0 for demand in demands):
            errors[
                "only_positive_demands"
            ] = "Negative demands are not allowed."

        if max_capacity <= 0:
            errors[
                "negative_max_capacity"
            ] = "Max capacity must be greater than 0."

        return errors

    @staticmethod
    def get_normalize_instance_parameters(
        int_depot, lst_clients, dict_demands, dict_loc_x, dict_loc_y
