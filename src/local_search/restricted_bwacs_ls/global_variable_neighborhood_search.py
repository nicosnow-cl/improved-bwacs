class RestrictedGlobalGVNS:
    ...
    def singleRouteRelocate(self, best_sr_reloc_sol, best_sr_reloc_qual, route_tabu_list):
        if len(best_sr_reloc_sol) >= 4:
            ...
            idx_i, idx_j = self.random.sample(
                list(range(len(temp_route)))[1:-1], 2)
    ...
    def multiRouteSwap(self, best_mr_swap_sol, best_mr_swap_qual, idx_r_i, idx_r_j):
        ...
        while (not is_feasible) and (n < max_n):
            if (len(best_mr_swap_sol[idx_r_i]) >= 3) and (len(best_mr_swap_sol[idx_r_j]) >= 3):
                idx_r_i, idx_r_j = self.random.sample(
                    range(len(best_mr_swap_sol)), 2)
                temp_route_a, temp_route_b = best_mr_swap_sol[idx_r_i].copy(
                ), best_mr_swap_sol[idx_r_j].copy()
                idx_n_i = self.random.choice(
                    list(range(len(temp_route_a)))[1:-1])
                idx_n_j = self.random.choice(
                    list(range(len(temp_route_b)))[1:-1])
                ...
                if (temp_route_a not in tabu_list) or (temp_route_b not in tabu_list):
                    ...
                    routes_demands = self.calculateRoutesDemands(
                        [temp_route_a, temp_route_b])
                    ...
                    if (routes_demands[0] <= self.vehicle_capacity) and (routes_demands[1] <= self.vehicle_capacity):
                        ...
                        new_quality = self.calculateSolutionQuality(
                            [temp_route_a, temp_route_b])

                        if new_quality.sum() < min_quality:
                            best_mr_swap_sol[idx_r_i], best_mr_swap_sol[idx_r_j] = self.deepcopy(
                                temp_route_a), self.deepcopy(temp_route_b)
                            best_mr_swap_qual[idx_r_i], best_mr_swap_qual[idx_r_j] = self.deepcopy(
                                new_quality[0]), self.deepcopy(new_quality[1])
                            is_feasible = True
            n += 1
    ...
    def shakingMultiRouteSwap(self, best_mr_swap_sol, best_mr_swap_qual):
        ...
        while (not is_feasible) and (n < max_n):
            ...
            if (len(best_mr_swap_sol[idx_routes[0]]) >= 3) and (len(best_mr_swap_sol[idx_routes[1]]) >= 3):
                temp_route_a, temp_route_b = self.deepcopy(
                    best_mr_swap_sol[idx_routes[0]]), self.deepcopy(best_mr_swap_sol[idx_routes[1]])
                idx_n_i = self.random.choice(
                    list(range(len(temp_route_a)))[1:-1])
                idx_n_j = self.random.choice(
                    list(range(len(temp_route_b)))[1:-1])
                ...
                if (temp_route_a not in self.tabu_list) or (temp_route_b not in self.tabu_list):
                    ...
                    if (routes_demands[0] <= self.vehicle_capacity) and (routes_demands[1] <= self.vehicle_capacity):
                        new_quality = self.calculateSolutionQuality(
                            [temp_route_a, temp_route_b])
                        best_mr_swap_sol[idx_routes[0]], best_mr_swap_sol[idx_routes[1]
                                                                          ] = temp_route_a, temp_route_b
                        best_mr_swap_qual[idx_routes[0]], best_mr_swap_qual[idx_routes[1]
                                                                            ] = new_quality[0], new_quality[1]
                        is_feasible = True
                    else:
    ...
    def shakingMultiRoute3Swap(self, best_mr_3swap_sol, best_mr_3swap_qual):
        ...
        while (not is_feasible) and (n < max_n):
            ...
            if ((len(best_mr_3swap_sol[idx_routes[0]]) >= 3) and (len(best_mr_3swap_sol[idx_routes[1]]) >= 3)
                ...
                temp_r_a, temp_r_b, temp_r_c = best_mr_3swap_sol[idx_routes[0]].copy(
                ), best_mr_3swap_sol[idx_routes[1]].copy(), best_mr_3swap_sol[idx_routes[2]].copy()
                ...
                if (temp_r_a not in self.tabu_list) or (temp_r_b not in self.tabu_list) or (temp_r_c not in self.tabu_list):
                    ...
                    if ((routes_demands[0] <= self.vehicle_capacity) and (routes_demands[1] <= self.vehicle_capacity) and
                        ...
                        new_quality = self.calculateSolutionQuality(
                            [temp_r_a, temp_r_b, temp_r_c])
                        best_mr_3swap_sol[idx_routes[0]], best_mr_3swap_sol[idx_routes[1]
                                                                            ], best_mr_3swap_sol[idx_routes[2]] = temp_r_a, temp_r_b, temp_r_c
                        best_mr_3swap_qual[idx_routes[0]], best_mr_3swap_qual[idx_routes[1]
                                                                              ], best_mr_3swap_qual[idx_routes[2]] = new_quality[0], new_quality[1], new_quality[2]
                        is_feasible = True
                    else:
    ...
    def shakingMultiRoute2Exchange(self, best_mr_2exc_sol, best_mr_2exc_qual):
        ...
        while (not is_feasible) and (n < max_n):
            ...
            if (len(best_mr_2exc_sol[idx_routes[0]]) >= 5) and (len(best_mr_2exc_sol[idx_routes[1]]) >= 5):
                temp_r_a, temp_r_b = best_mr_2exc_sol[idx_routes[0]].copy(
                ), best_mr_2exc_sol[idx_routes[1]].copy()
                idx_n_i = self.random.choice(list(range(len(temp_r_a)))[1:-2])
                idx_n_j = self.random.choice(list(range(len(temp_r_b)))[1:-2])
                temp_r_a[idx_n_i], temp_r_a[idx_n_i+1], temp_r_b[idx_n_j], temp_r_b[idx_n_j +
                                                                                    1] = temp_r_b[idx_n_j], temp_r_b[idx_n_j+1], temp_r_a[idx_n_i], temp_r_a[idx_n_i+1]
                ...
                if (temp_r_a not in self.tabu_list) and (temp_r_b not in self.tabu_list):
                    ...
                    if (routes_demands[0] <= self.vehicle_capacity) and (routes_demands[1] <= self.vehicle_capacity):
                        new_quality = self.calculateSolutionQuality(
                            [temp_r_a, temp_r_b])
                        best_mr_2exc_sol[idx_routes[0]
                                         ], best_mr_2exc_sol[idx_routes[1]] = temp_r_a, temp_r_b
                        best_mr_2exc_qual[idx_routes[0]], best_mr_2exc_qual[idx_routes[1]
                                                                            ] = new_quality[0], new_quality[1]
                        is_feasible = True
                    else:
                        self.tabu_list.append(temp_r_a)
    ...
    def shakingMultiRoute3Exchange(self, best_mr_3exc_sol, best_mr_3exc_qual):
        ...
        while (not is_feasible) and (n < max_n):
            ...
            if ((len(best_mr_3exc_sol[idx_routes[0]]) >= 5) and (len(best_mr_3exc_sol[idx_routes[1]]) >= 5)
                ...
                temp_r_a, temp_r_b, temp_r_c = best_mr_3exc_sol[idx_routes[0]].copy(
                ), best_mr_3exc_sol[idx_routes[1]].copy(), best_mr_3exc_sol[idx_routes[2]].copy()
                idx_n_i = self.random.choice(list(range(len(temp_r_a)))[1:-2])
                idx_n_j = self.random.choice(list(range(len(temp_r_b)))[1:-2])
                idx_n_k = self.random.choice(list(range(len(temp_r_c)))[1:-2])
                temp_r_a[idx_n_i], temp_r_a[idx_n_i+1], temp_r_b[idx_n_j], temp_r_b[idx_n_j+1], temp_r_c[idx_n_k], temp_r_c[idx_n_k +
                                                                                                                            1] = temp_r_c[idx_n_k], temp_r_c[idx_n_k+1], temp_r_a[idx_n_i], temp_r_a[idx_n_i+1], temp_r_b[idx_n_j], temp_r_b[idx_n_j+1]
                ...
                if (temp_r_a not in self.tabu_list) and (temp_r_b not in self.tabu_list) and (temp_r_c not in self.tabu_list):
                    ...
                    if ((routes_demands[0] <= self.vehicle_capacity) and (routes_demands[1] <= self.vehicle_capacity)
                       and (routes_demands[2] <= self.vehicle_capacity)):
                        new_quality = self.calculateSolutionQuality(
                            [temp_r_a, temp_r_b, temp_r_c])
                        best_mr_3exc_sol[idx_routes[0]], best_mr_3exc_sol[idx_routes[1]
                                                                          ], best_mr_3exc_sol[idx_routes[2]] = temp_r_a, temp_r_b, temp_r_c
                        best_mr_3exc_qual[idx_routes[0]], best_mr_3exc_qual[idx_routes[1]
                                                                            ], best_mr_3exc_qual[idx_routes[2]] = new_quality[0], new_quality[1], new_quality[2]
                        is_feasible = True
                    else:
                        self.tabu_list.append(temp_r_a)
                        self.tabu_list.append(temp_r_b)
    ...
    def shaking(self, original_solution, original_quality, N):
        if (N == 0):
            return original_solution, original_quality, list(range(len(original_solution)))
        elif (N == 1):
            return self.shakingMultiRouteSwap(original_solution.copy(), original_quality.copy())
        elif (N == 2):
            return self.shakingMultiRoute3Swap(original_solution.copy(), original_quality.copy())
        elif (N == 3):
            return self.shakingMultiRoute2Exchange(original_solution.copy(), original_quality.copy())
        else:
            return self.shakingMultiRoute3Exchange(original_solution.copy(), original_quality.copy())
    ...
    def VNS(self, shaking_solution, shaking_quality, N, idx_routes):
        ...
        for idx_route in idx_routes:
            ...
            for n in range(N):
                best_vns_solution[idx_route], best_vns_quality[idx_route], route_tabu_list = self.singleRouteSwap(
                    best_vns_solution[idx_route].copy(), best_vns_quality[idx_route].copy(), route_tabu_list)
                best_vns_solution[idx_route], best_vns_quality[idx_route], route_tabu_list = self.singleRouteRelocate(
                    best_vns_solution[idx_route].copy(), best_vns_quality[idx_route].copy(), route_tabu_list)
    ...
    def improve(self):
        ...
        new_improve = False
        max_time = len(self.demands_array) * 0.75

        start_time = time.time()
        print(str(time.time() - start_time) +
              ': ' + str(best_global_quality.sum()))
        while True:
            for shaking_ratio in range(5):
                shaking_solution, shaking_quality, idx_routes = self.shaking(best_global_solution.copy(), best_global_quality.copy(),
                                                                             shaking_ratio)
                vns_solution, vns_quality = self.VNS(
                    shaking_solution, shaking_quality, int(len(self.demands_array) / 2), idx_routes)
