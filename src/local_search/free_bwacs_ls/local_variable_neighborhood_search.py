class FreeLocalGVNS:
    ...
    def singleRouteRelocate(self, best_sr_reloc_sol, best_sr_reloc_qual, idx_route=None, tabu_list=[]):
        if (idx_route == None):
            idx_route = self.random.choice(range(len(best_sr_reloc_sol)))
        ...
        while (not is_feasible) and (n < max_n):
            ...
            if (len(best_mr_reloc_sol[idx_routes[0]]) >= 4):
                min_quality = ((best_mr_reloc_qual[idx_routes[0]] + best_mr_reloc_qual[idx_routes[1]])
                               + ((best_mr_reloc_qual[idx_routes[0]] + best_mr_reloc_qual[idx_routes[1]]) * 0.1))

                temp_route_a, temp_route_b = best_mr_reloc_sol[idx_routes[0]].copy(
                ), best_mr_reloc_sol[idx_routes[1]].copy()
                ...
                if (temp_route_b not in tabu_list):
                    ...
                    if (routes_demands[0] <= self.vehicle_capacity):
                        ...
                        if new_quality.sum() < min_quality:
                            best_mr_reloc_sol[idx_routes[0]], best_mr_reloc_sol[idx_routes[1]
                                                                                ] = temp_route_a, temp_route_b
                            best_mr_reloc_qual[idx_routes[0]], best_mr_reloc_qual[idx_routes[1]
    ...
    def shakingMultiRouteSwap(self, best_mr_swap_sol, best_mr_swap_qual):
        ...
        while (not is_feasible) and (n < max_n):
            ...
            if (len(best_mr_swap_sol[idx_routes[0]]) >= 3) and (len(best_mr_swap_sol[idx_routes[1]]) >= 3):
                ...
                temp_route_a, temp_route_b = best_mr_swap_sol[idx_routes[0]].copy(
                ), best_mr_swap_sol[idx_routes[1]].copy()
                idx_n_i = self.random.choice(
                    list(range(len(temp_route_a)))[1:-1])
                idx_n_j = self.random.choice(
                    list(range(len(temp_route_b)))[1:-1])
    ...
    def shakingMultiRoute2Exchange(self, best_mr_2exc_sol, best_mr_2exc_qual):
        ...
        while (not is_feasible) and (n < max_n):
            ...
            if (len(best_mr_2exc_sol[idx_routes[0]]) >= 5) and (len(best_mr_2exc_sol[idx_routes[1]]) >= 5):
                ...
                temp_route_a, temp_route_b = best_mr_2exc_sol[idx_routes[0]].copy(
                ), best_mr_2exc_sol[idx_routes[1]].copy()
                idx_n_i = self.random.choice(
                    list(range(len(temp_route_a)))[1:-2])
                idx_n_j = self.random.choice(
                    list(range(len(temp_route_b)))[1:-2])
                temp_route_a[idx_n_i], temp_route_a[idx_n_i+1], temp_route_b[idx_n_j], temp_route_b[idx_n_j +
                                                                                                    1] = temp_route_b[idx_n_j], temp_route_b[idx_n_j+1], temp_route_a[idx_n_i], temp_route_a[idx_n_i+1]
                ...
                if (temp_route_a not in tabu_list) or (temp_route_b not in tabu_list):
                    ...
                    if (routes_demands[0] <= self.vehicle_capacity) and (routes_demands[1] <= self.vehicle_capacity):
                        ...
                        if new_quality.sum() < min_quality:
                            best_mr_2exc_sol[idx_routes[0]], best_mr_2exc_sol[idx_routes[1]
                                                                              ] = temp_route_a, temp_route_b
    ...
    def shaking(self, original_solution, original_quality, N):
        if (N == 0):
            ...
            return self.shakingMultiRouteRelocate(original_solution.copy(), original_quality.copy())
        elif (N == 2):
    ...
    def VNS(self, shaking_solution, shaking_quality, best_global_quality, N, idx_routes):
        ...
        for idx_route in idx_routes:
            ...
            for n in range(N):
                N = len(best_vns_solution[idx_route])
                # singleRouteSwap()
                best_vns_solution, best_vns_quality, tabu_list = self.singleRouteSwap(best_vns_solution.copy(), best_vns_quality.copy(),
                                                                                      idx_route, tabu_list)
                # singleRouteRelocate()
    ...
    def improve(self):
        ...
        if self.intensity_percentage <= 0.85:
            # max_time = len(self.demands_array) * 0.005
            max_time = len(self.demands_array) * 0.01
        else:
            # max_time = len(self.demands_array) * 0.0075
            max_time = len(self.demands_array) * 0.015
        ...
        while i < self.k_number:
            for shaking_ratio in range(4):
                ...
                vns_solution, vns_quality = self.VNS(shaking_solution.copy(), shaking_quality.copy(), best_global_quality.copy(),
                                                     int(((len(self.demands_array) - 1) / 3)), idx_routes)
