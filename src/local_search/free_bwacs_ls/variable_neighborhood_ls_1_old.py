class FreeBWACSGVN:
    ...
    def multiRoute3Swap(self, best_mr_3swap_sol, best_mr_3swap_qual, idx_r_i = None, idx_r_j = None, idx_r_k = None):
        if (idx_r_i == None) or (idx_r_j == None) or (idx_r_k == None): range_solution = range(len(best_mr_3swap_sol))
        tabu_list = []
        is_feasible = False
        n = 1
        ...
        while (not is_feasible) and (n < max_n):
            if (idx_r_i == None) or (idx_r_j == None) or (idx_r_k == None): idx_r_i, idx_r_j = self.random.sample(range_solution, 3)
        ...
        while (not is_feasible) and (n < max_n):
            ...
            if (len(best_mr_swap_sol[idx_routes[0]]) >= 3) and (len(best_mr_swap_sol[idx_routes[1]]) >= 3):
                min_quality = ((best_mr_swap_qual[idx_routes[0]] + best_mr_swap_qual[idx_routes[1]]) 
                           + ((best_mr_swap_qual[idx_routes[0]] + best_mr_swap_qual[idx_routes[1]]) * 0.3))
    ...
    def shaking(self, original_solution, original_quality, N):        
        ...
        elif (N == 1):                   return self.shakingMultiRouteSwap(original_solution, original_quality)
        ...
        if (len(idx_routes) == 2):
            ...
            for idx_r_i, idx_r_j in idx_perm:
                ...
                for n in range(N):
                    ...
                    best_vns_solution, best_vns_quality = self.singleRouteRelocate(best_vns_solution, best_vns_quality, 
                                                                                   idx_r_j)
                if best_vns_quality.sum() < best_global_quality.sum(): return best_vns_solution, best_vns_quality
                
        elif (len(idx_routes) == 3):
            idx_perm = list(permutations(idx_routes, 3))
            for idx_r_i, idx_r_j, idx_r_k in idx_perm:
                for n in range(int(N * 0.50)):    
                   # multiRoute3Swap()
                   best_vns_solution, best_vns_quality = self.multiRoute3Swap(best_vns_solution, best_vns_quality, 
                                                                             idx_r_i, idx_r_j, idx_r_k)           
                for n in range(N):
                    # singleRouteRelocate()            
                    best_vns_solution, best_vns_quality = self.singleRouteRelocate(best_vns_solution, best_vns_quality, 
                                                                                   idx_r_i)
    ...
    def improve(self):        
        ...
        if self.intensity_percentage <= 0.85: 
            max_time = len(self.demands_array) * 0.003
        else:
            max_time = len(self.demands_array) * 0.006
            
        start_time = time.time()
        while i < (self.k_number * 2):
            for shaking_ratio in range(3):
                ...
                vns_solution, vns_quality = self.VNS(shaking_solution, shaking_quality, best_global_quality, 
                                                     int((len(self.demands_array) / 2)), idx_routes)     
