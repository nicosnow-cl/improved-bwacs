class ReaderCVRPLIB:
    def __init__(self, file_name, path='', max_nodes=99999):
        import os
        import math
        ...
        if self.file_name:
            ...
            try:
                with open(self.path + self.file_name + '.vrp', 'r') as f:
                    ...
                    clients = list(data['NODE_COORD_SECTION'].keys())[
                        1:self.max_nodes]
                    loc_x = {node: data['NODE_COORD_SECTION'][node][0]
                             for node in [depot] + clients}
                    loc_y = {node: data['NODE_COORD_SECTION'][node][1]
                             for node in [depot] + clients}
                    demands = {node: data['DEMAND_SECTION'][node]
                               for node in [depot] + clients}
