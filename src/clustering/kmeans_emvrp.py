class KMeansEMVRP:
    ...
    def getDList(self):
        idx_centroids_list = []
        idx_centroids_list.append((self.distances_matrix[self.depot, :] * self.demands_array).argmax())
        # idx_centroids_list.append(self.distances_matrix[self.depot, :].argmax())
        
        for k in range(self.k_number - 1):
            _priority = self.distances_matrix[:, [self.depot] + idx_centroids_list]
    ...
    def calculateDistancesToCentroids(self):
        self.distances_to_centroids_matrix = self.np.zeros((self.k_number, len(self.nodes)))
        ord_ = 1 if self.metric == 'manhattan' else 2
        
        for k, c in enumerate(self.D):
            self.distances_to_centroids_matrix[k] = [self.np.linalg.norm(self.coords_matrix[i] - c, ord = ord_) for i in self.nodes]

    def calculateTotalCost(self, clusters):
        total_cost = 0
        
        for k, cluster in enumerate(clusters):
            cluster_energy = self.distances_to_centroids_matrix[k][cluster] * self.demands_array[cluster] 
    ...
    def calculateNewCentroids(self, unassigned_nodes, clusters):
        D_list = self.np.array([self.coords_matrix[cluster].mean(axis = 0) for cluster in clusters])
        
        for node in unassigned_nodes:
            M = self.distances_to_centroids_matrix[:, node] * self.demands_array[node]
    ...
    def run(self):
        ...
        if self.distances_matrix.shape[0] < 25:                                               limit_constraint = 2    
        elif self.distances_matrix.shape[0] >= 25 and self.distances_matrix.shape[0] < 50:    limit_constraint = 2  
        elif self.distances_matrix.shape[0] >= 50 and self.distances_matrix.shape[0] < 75:    limit_constraint = 2
        elif self.distances_matrix.shape[0] >= 75 and self.distances_matrix.shape[0] < 100:   limit_constraint = 1.5
        elif self.distances_matrix.shape[0] >= 100 and self.distances_matrix.shape[0] < 150:  limit_constraint = 1
        elif self.distances_matrix.shape[0] >= 150 and self.distances_matrix.shape[0] < 250:  limit_constraint = 0.8
        elif self.distances_matrix.shape[0] >= 250 and self.distances_matrix.shape[0] < 500:  limit_constraint = 0.4
        elif self.distances_matrix.shape[0] >= 500 and self.distances_matrix.shape[0] < 1000: limit_constraint = 0.1
        else:                                                                                 limit_constraint = 0.05
        ...
        while (iteration <= max_iterations) or (best_total_cost == self.np.inf):
            ...
            print('        - Centroides actuales: ' + str([coords for coords in self.D]))
            
            for ri in self.nodes:
                if ri in unassigned_nodes:                    
                    M = self.distances_to_centroids_matrix[:, ri] * self.demands_array[ri]
                    M = M.argsort() 
                    
                    for m in M:              
                        cost_submatrix = self.distances_to_centroids_matrix[m, unassigned_nodes]
                        
                        for pos, i in enumerate(unassigned_nodes):
                            cost_submatrix[pos] *= self.demands_array[i]
                        
                        priority_matrix = cost_submatrix / self.demands_array[unassigned_nodes]
            ...
            self.D = self.calculateNewCentroids(unassigned_nodes, self.clusters)
            ...
            if (not unassigned_nodes.size) and (new_total_cost < best_constraint_total_cost):
                ...
                for k in range(self.k_number):
                    best_constraint_clusters[k] = self.np.array(best_constraint_clusters[k])
